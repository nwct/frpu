...
Options:
    ...
    --addr=<bind_addr>        listen addr for client, example: 0.0.0.0:7100
...
func main() {
	...
	if err != nil {
		fmt.Println(err)
		fmt.Println("未找到指定的配置文件，已经使用默认配置启动程序")
		fmt.Println("")
		fmt.Println("默认HTTP端口为：80")
		fmt.Println("默认HTTPS端口为：443")
		fmt.Println("默认连接端口为：7100")
		fmt.Println("状态查询端口为：7600")
		fmt.Println("默认特权密码为：frp888")
		fmt.Println("")
		fmt.Println("更多帮助请访问：http://www.lu8.top")
		fmt.Println("")
		//	os.Exit(1)
	}
	...
	if args["--addr"] != nil {
		...
		if len(addr) != 2 {
			fmt.Println("--addr format error: example 0.0.0.0:7100")
			os.Exit(1)
		}
